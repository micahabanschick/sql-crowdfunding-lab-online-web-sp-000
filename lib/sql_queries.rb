...
def selects_the_titles_and_amount_over_goal_of_all_projects_that_have_met_their_funding_goal
    "SELECT projects.title, SUM(pledges.amount) - projects.funding_goal AS total FROM projects LEFT OUTER JOIN pledges ON pledges.project_id = projects.id WHERE total >= 0 GROUP BY pledges.project_id"
end
