...
def selects_the_titles_and_amount_over_goal_of_all_projects_that_have_met_their_funding_goal
  "SELECT projects.title, (pledges.amount - projects.funding_goal) FROM projects LEFT OUTER JOIN pledges ON pledges.project_id = projects.id WHERE pledges.amount >= projects.funding_goal GROUP BY pledges.amount HAVING (pledges.amount - projects.funding_goal) >= 0"
end
